...
namespace Engine5
{
    MultiTextureShaderDX11::MultiTextureShaderDX11()
    {
    }
    ...
    void MultiTextureShaderDX11::SetHWnd(HWND hwnd)
    {
        m_hwnd = hwnd;
    }

    void MultiTextureShaderDX11::SetDevice(ID3D11Device* device)
    {
        m_device = device;
    }

    void MultiTextureShaderDX11::SetDeviceContext(ID3D11DeviceContext* device_context)
    {
        m_device_context = device_context;
    }

    MultiTextureShaderCommon::MultiTextureShaderCommon(ShaderManagerCommon* shader_manager)
        : m_shader_manager(shader_manager)
    {
    }

    MultiTextureShaderCommon::~MultiTextureShaderCommon()
    {
    }

    void MultiTextureShaderCommon::SetShader(ShaderResource* shader)
    {
        m_shader_resource = shader;
    }

    bool MultiTextureShaderCommon::Initialize()
    {
        ...
        HRESULT result = D3DCompileFromFile(vertex_shader_path.c_str(), nullptr, nullptr, "TextureVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertex_shader_buffer, &error_message);
        if (FAILED(result))
        {
        ...
        result = D3DCompileFromFile(pixel_shader_path.c_str(), nullptr, nullptr, "TexturePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixel_shader_buffer, &error_message);
        if (FAILED(result))
        {
            // If the shader failed to compile it should have written something to the error message.
            if (error_message)
        ...
        D3D11_BUFFER_DESC gamma_buffer_desc;
        gamma_buffer_desc.Usage               = D3D11_USAGE_DYNAMIC;
        gamma_buffer_desc.ByteWidth           = sizeof(GammaBufferType);
        ...
        // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
        result = m_device->CreateBuffer(&gamma_buffer_desc, nullptr, &m_gamma_buffer);
        if (FAILED(result))
        {
            return false;
        }
    ...
    void MultiTextureShaderCommon::Render(U32 indices_count, const MatrixData& mvp_data, TextureArrayCommon* texture, const Color& color, Real gamma) const
    {
        ...
        result = m_device_context->Map(m_gamma_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
        if (FAILED(result))
        {
            return;
        }
        // Get a pointer to the data in the constant buffer.
        GammaBufferType* gamma_data_ptr = (GammaBufferType*)mapped_resource.pData;
        // Copy the lighting variables into the constant buffer.
        gamma_data_ptr->gamma_correction = gamma;
        // Unlock the constant buffer.
        m_device_context->Unmap(m_gamma_buffer, 0);
        // Set the position of the light constant buffer in the pixel shader.
        buffer_number = 1;
        // Finally set the light constant buffer in the pixel shader with the updated values.
        m_device_context->PSSetConstantBuffers(buffer_number, 1, &m_gamma_buffer);
        //render
    ...
    void MultiTextureShaderCommon::Shutdown()
    {
        ...
        // Release the gamma constant buffer.
