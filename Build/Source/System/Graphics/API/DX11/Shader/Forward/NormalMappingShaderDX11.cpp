...
namespace Engine5
{
    NormalMappingShaderDX11::NormalMappingShaderDX11()
    {
    }
    ...
    void NormalMappingShaderDX11::SetHWnd(HWND hwnd)
    {
        m_hwnd = hwnd;
    }

    void NormalMappingShaderDX11::SetDevice(ID3D11Device* device)
    {
        m_device = device;
    }

    void NormalMappingShaderDX11::SetDeviceContext(ID3D11DeviceContext* device_context)
    {
        m_device_context = device_context;
    }

    NormalMappingShaderCommon::NormalMappingShaderCommon(ShaderManagerCommon* shader_manager)
        : m_shader_manager(shader_manager)
    {
    }

    NormalMappingShaderCommon::~NormalMappingShaderCommon()
    {
    }

    void NormalMappingShaderCommon::SetShader(ShaderResource* shader)
    {
        m_shader_resource = shader;
    }

    bool NormalMappingShaderCommon::Initialize()
    {
        ...
        gamma_buffer_desc.ByteWidth           = sizeof(LightBufferType);
        ...
        // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
        result = m_device->CreateBuffer(&gamma_buffer_desc, nullptr, &m_light_buffer);
        if (FAILED(result))
        {
            return false;
        }
    ...
    void NormalMappingShaderCommon::Render(U32 indices_count, const MatrixData& mvp_data, TextureArrayCommon* texture, const Color& color, DirectionalLight* light) const
    {
    ...
    void NormalMappingShaderCommon::Shutdown()
    {
