...
namespace Engine5
{
    ...
    bool RenderTextureBufferDX11::BuildBuffer(Real left, Real right, Real top, Real bottom)
    {
        m_vertex_count = 4;
        m_index_count = 6;
    ...
    bool RenderTextureBufferCommon::Initialize(RendererCommon* renderer, size_t width, size_t height)
    {
        ...
        return BuildBuffer(left, right, top, bottom);
    }

    bool RenderTextureBufferCommon::Initialize(RendererCommon* renderer, Real left, Real right, Real top, Real bottom)
    {
        SetDevice(renderer->GetDevice());
        SetDeviceContext(renderer->GetDeviceContext());
        return BuildBuffer(left, right, top, bottom);
    ...
    bool RenderTextureBufferCommon::OnResize(size_t width, size_t height)
    {
        ...
        Real top = half_y;
        Real bottom = -half_y;
        return OnResize(left, right, top, bottom);
    }

    bool RenderTextureBufferCommon::OnResize(Real left, Real right, Real top, Real bottom)
    {
        Shutdown();
        return BuildBuffer(left, right, top, bottom);
    }
